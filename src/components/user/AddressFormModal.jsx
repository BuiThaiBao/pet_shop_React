...
const AddressFormModal = ({ isOpen, onClose, onSave }) => {
	...
	const validateForm = () => {
		...
		if (!form.district.trim()) newErrors.district = 'Quận/Huyện không được để trống';
		// ward is optional
		return newErrors;
	};
		...
		<ModalShell
			...
			footer={(
				<>
					...
					<button type="submit" form="addressForm" className="btn btn-primary" disabled={isSubmitting}>
			...
			<form id="addressForm" onSubmit={handleSubmit}>
				<div className="row g-3">
					...
					<div className="col-md-4">
						...
						<input className={`form-control ${errors.city ? 'is-invalid' : ''}`} name="city" value={form.city} onChange={handleChange} />
						{errors.city && <div className="invalid-feedback">{errors.city}</div>}
					</div>
					<div className="col-md-4">
						<label className="form-label">Quận/Huyện</label>
						<input className={`form-control ${errors.district ? 'is-invalid' : ''}`} name="district" value={form.district} onChange={handleChange} />
